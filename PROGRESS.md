...
**多轮对话(2026-07-16)**:/api/ask 接 history(裁最近 3 轮、单条截 1200 字);检索融合上一问 + 上一答开头(序数指代的对象在上一条回答里);系统提示词加多轮规则(序数以助手上一条列举顺序为准);前端改消息线程 UI(用户/助手气泡、来源挂各自消息、新对话按钮、流式草稿、失败清半截草稿)。E2E 实测:「支持哪些部署方式?」→「第二种的前置要求?」正确解析为集群部署并给出带来源的回答。
